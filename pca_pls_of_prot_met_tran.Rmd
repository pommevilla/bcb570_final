...
plot(pca.transcriptome) + title(main="transcriptomics PCA Plot")
```
...
```{r}
prot_tran_genotypes<-t(cbind(proteomic.transcriptomic.merged[8:16],
                             proteomic.transcriptomic.merged[21:29]))
colnames(prot_tran_genotypes)<-proteomic.transcriptomic.merged$UNIQID
pca.proteome_transcriptome <- pca(prot_tran_genotypes, ncomp = 10, center = TRUE, scale = TRUE)

pca.proteome_transcriptome

plot(pca.proteome_transcriptome) + title(main="Protein and Transcriptomic PCA Plot")
...
```{r}
pca_nth_component_histogram(pca.proteome_transcriptome, 2)
```

```{r}
pca.proteome$loadings$X
```

```{r}
get_top_loadings(pca.proteome, 1) %>% names
```
...
plot(tune.pls$Q2.total) + abline(h = 0.0975)
