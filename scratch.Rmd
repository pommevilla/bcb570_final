...
```{r}
proteomics.cor <- threshold_cor_matrix(proteomics.cor, quantiles)
ggplot(melt(proteomics.cor), aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
  coord_equal() +
...
```{r}
write.csv(temp, "./data/grn_mats/proteomics_network.csv", quote = FALSE,
          row.names = FALSE)
```
...
proteomics.g <- graph_from_adjacency_matrix(proteomics.cor, diag = FALSE
